...
class UserDataAndAlerts:
    def __init__(self, user_id, context, forms=None):
        #  Get the user profile data
        MemberProfileData(user_id, context, forms)
        # Get all user alerts
        UserAlerts(user_id, context)
...
class MemberProfileData:
    def __init__(self, user_id, context, forms=None):
        # Get the user profile data
        member_profile = QuerySet.using_get(
            model_name=models.Profiles, user_id=user_id
        )
        context["member_address"] = GetCityAndCountry.names(
            member_profile.depart_city_id, member_profile.depart_country_id
        )
        context["profile"] = member_profile

        if forms is not None:
            context["edit_form"] = forms.RegisterForm(
...
class MemberPosts:
    def __init__(self, user_id, context):
        ...
        df_user_posts['nbr_current_res'] = 0
        df_user_posts['nbr_completed_res'] = 0
        df_user_posts['nbr_canceled_res'] = 0
        if not df_user_posts.empty:
            ...
            if not df_query_of_reservations.empty:
                # Adding same change to df_query_set dataframe
                df_reserved = GetReservations.as_dataframe(df_query_of_reservations)
        ...
        df_member_current_posts = MemberCurrentPosts.as_dataframe(df_user_posts)
...
class GetPostsData:
    ...
    def as_dataframe(dataframe):
        ...
        if not df_post.empty:
            df_post.sort_values(by=["publish_date"], inplace=True, ascending=True)
            ...
            for poster_id in posters_id:
                ...
                # col = "poster_id"
                keys = [
                    "p_full_name", "p_notes", "p_accept_rate", "p_cancel_rate", "p_profile_img",
                    "p_email", "p_phone_number", "p_language", "p_date_joined"
                ]
                values = [
                    user.full_name, profile.notes, profile.accept_rate, profile.cancel_rate, profile.image.url,
                    user.email, profile.phone_number, profile.language, user.date_joined
                ]
...
class PostDetail:
    ...
    def as_dataframe(post_id):
        ...
        posts_query_set = QuerySet.using_filter(
            models.Posts, **{'id': post_id}
        )

        df_post_detail = GetPostsData.as_dataframe(
            posts_query_set['query_as_dataframe']
        )
...
class MemberCurrentPosts:
    ...
    def as_dataframe(posts_as_dataframe):
        state = variabless.current
        df_current_member_posts = posts_as_dataframe[posts_as_dataframe["post_state"] == state]
...
class MemberCompletedPosts:
    ...
    def as_dataframe(posts_as_dataframe):
        ...
        df_completed_member_posts = posts_as_dataframe[posts_as_dataframe['post_state'] == state]
...
class MemberCanceledPosts:
    ...
    def as_dataframe(posts_as_dataframe):
        ...
        df_canceled_member_posts = posts_as_dataframe[posts_as_dataframe['post_state'] == state]
...
class MemberReservations:
    def __init__(self, user_id, context):
        ...
        df_member_reservation = GetReservations.as_dataframe(df_query_set)
    ...
    def as_dataframe(df_reservation):
        if not df_reservation.empty:
            ...
            for reserver_id in reserver_id_list:
                ...
                keys = [
                    'r_full_name', 'r_notes', 'r_accept_rate', 'r_cancel_rate', 'r_profile_img', 'r_email', 'r_phone_number', 'r_language'
                ]
                values = [
...
class MemberCurrentReservations:
    ...
    def as_dataframe(reservations_as_dataframe):
        state = variabless.current
        df_current_member_reservations = reservations_as_dataframe[reservations_as_dataframe['reserv_state'] == state]
...
class MemberCompletedReservations:
    ...
    def as_dataframe(reservations_as_dataframe):
        state = variabless.completed
        df_completed_member_reservations = reservations_as_dataframe[reservations_as_dataframe['reserv_state'] == state]
...
class MemberCanceledReservations:
    ...
    def as_dataframe(reservations_as_dataframe):
        state = variabless.canceled
        df_canceled_member_reservations = reservations_as_dataframe[reservations_as_dataframe['reserv_state'] == state]
...
class Country:
    ...
    def name(country_id=None, country_name=None):
        try:
            if country_id is not None:
                ...
                country = models.Countries.objects.get(name=country_name)
            else:
                country = None
        except models.Countries.DoesNotExist:
            country = None
    ...
    @staticmethod
    def currency(country_id):
        try:
            country_obj = models.Countries.objects.get(id=country_id)
...
class ListOfCities:
    ...
    def name(country_id):
        try:
            cities = models.Cities.objects.filter(country_id=country_id)
...
class GetCityAndCountry:
    @staticmethod
    def names(city_id, country_id):
        city_and_country_name = '{}, {}'.format(
            City.name(city_id),  # Get the city name from city id
            Country.name(country_id)  # Get the country name from country id
        )
...
class QuerySet:
    ...
    def using_filter(model_name, get_n_rows=None, list_of_id=None, **kwargs):
        ...
        try:
            if list_of_id is not None:
                # Get the query set
                query_set = model_name.objects.filter(id__in=list_of_id)
            else:
                query_set = model_name.objects.filter(**kwargs)
        ...
        if query_set.exists():
            if list_of_id is not None:
                df_query_set = pd.DataFrame(list(query_set.values()))
    ...
    def using_get(model_name, **kwargs):
        try:
            query = model_name.objects.get(**kwargs)
        except model_name.DoesNotExist:
            query = None
...
class UserAlerts:
    def __init__(self, user_id, context):
        kwargs = CreateDictionary.from_two_lists(
            ["member_id", "alert_state"], [user_id, "In progress"]
        )
        alerts = QuerySet.using_filter(
            models.AlertMe, **kwargs
        )
        ...
        if not df_alerts.empty:
            ...
            # Change the countries' id to cities name
            for country_id in unique_countries_id:
                country_name = Country.name(country_id=country_id)
            ...
            df_alerts["departure_date"] = [MonthsName.departure_date(str(d)) for d in old_format]
            # Adding the poster information to dataframe
...
def current_data():
    ...
    year, month_index, day = today.split('-')[0], today.split('-')[1], today.split('-')[2]
    month = mois[int(month_index)-1]
    la_date = "{} {} {}".format(day, month, year)

    aujourdhui = datetime.now()
    leJour = date.weekday(aujourdhui)
    lesJours = ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche']

    date_format = "{}, {}".format(lesJours[leJour], la_date)
